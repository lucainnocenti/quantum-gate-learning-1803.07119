...
def make_alldiagonal_model(num_qubits):
    ...
    for index in indices:
        if len(list(filter(lambda a: a == 3, index))) <= 1:
            XY_indices.append(index)
    return ac.indices_to_hamiltonian(XY_indices)
...
def main():
    ...
    # interactions = 'all'
    fredkin_model = make_alldiagonal_model(num_qubits)
    # HYPERPARAMETERS
